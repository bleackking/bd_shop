...
import 'package:bd_shop/utils/Toastutils.dart';
...
class _HomeViewState extends State<HomeView> {
  ...
  List<Widget> _getScrollChildren() {
    return [
      ...
      SliverToBoxAdapter(child: SizedBox(height: 10)),
      // 商品无限滚动列表组件
      BdMoreList(recommendList: _recommendList),
    ];
  }
  ...
  // 获取一站式推荐列表数据
  Future<void> _getOneStepList() async {
    _oneStepResult = await getOneStepListAPI();
  }

  // 获取推荐列表数据
  Future<void> _getRecommendList() async {
    // 当已经有请求正在加载，或者没有下一页了，就放弃请求
  ...
  Future<void> _onRefresh() async {
    ...
    await _getOneStepList();
    await _getRecommendList();
    // 数据获取成功，数据刷新成功了
    ToastUtils.showToast(context, "刷新成功咯~");
    _paddingTop = 0;
    setState(() {});
  }
